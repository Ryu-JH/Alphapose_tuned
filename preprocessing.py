...
if __name__ == "__main__":
    in_path = '/home/ryu/Documents/posestream/nas_dms_dataset'
    drivers = ['GeesungOh', 'JoonghooPark', 'EuiseokJeong', 'TaesanKim']
    out_path = '/home/ryu/Documents/posestream/nas_dms_dataset/heatmap'
    for driver in drivers:
        odds = os.listdir(f'{in_path}/{driver}')
        for odd in odds:
            args.outputpath = f'{out_path}/{driver}/{odd}'
            args.video = f'{in_path}/{driver}/{odd}/video/SideView/ir_side2.avi'
            #path = '/home/ryu/Documents/posestream'
            print(f'checking {args.outputpath}')
            ...
            hm_data = np.zeros((data_len, 17, 64, 48))
            preds_data = np.zeros((data_len, 17, 2))
            ...
            try:
                for i in im_names_desc:
                    ...
                    preds_data[i] = preds
                print('Saving numpy....')
                np.save(f'{args.outputpath}/hm', hm_data)
                np.save(f'{args.outputpath}/keypoints', preds_data)
                print(data_len, hm_data.shape, preds_data.shape)
                print_finish_info()
                det_loader.stop()
